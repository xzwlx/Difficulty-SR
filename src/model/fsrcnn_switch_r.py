...
def make_model(args, parent=False):
    return FSRCNN_switch_r(args)

class FSRCNN_switch_r(nn.Module):
    def __init__(self, args, conv=common.default_conv, d=56, s=12, m=4):
        super(FSRCNN_switch_r, self).__init__()
        ...
        self.head_s = nn.Sequential(*m_head_s)
        self.body = nn.Sequential(*m_body)
        self.tail = nn.Sequential(*m_tail)
        self.tail_s = nn.Sequential(*m_tail_s)
